...
class PredictRasterAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        model_filename_rk = 'C:/Users/n.smit/OneDrive - Oosterhoff Group/Documents/Master Thesis/Coding/predictions_QGIS/model_rk_rfr_regressor_parks_roadnoise.joblib'
        # model_filename_rk = 'C:/Users/n.smit/OneDrive - Oosterhoff Group/Documents/Master Thesis/Coding/predictions_QGIS/model_rk_svm_regressor_parks_roadnoise.joblib'
        rf_regressor = joblib.load(model_filename)
        model = rf_regressor
        
        
        model_rk = joblib.load(model_filename_rk)
        ...
        features_all = ['left', 'top', 'right', 'bottom', 'row_index', 'col_index', 'Parksdist','OUTPUT']
        ...
        columns_all = ['left', 'top', 'right', 'bottom', 'row_index', 'col_index', 'PARKSDISTAN', 'roadnoiseco']
        columns = ['PARKSDISTAN', 'roadnoiseco']
        ...
        grid_points_data_x = grid_points_data[['lon', 'lat']].values.tolist()
        grid_points_data_x = np.array(grid_points_data_x)
        print(grid_points_data_x[0])
        print("type array", type(grid_points_data_x),"shape", grid_points_data_x.shape)
        # # Make predictions wit RFR
        # grid_points_predictions = model.predict(grid_points_data_input)
        
        # # Predictions with Regression Kriging
        grid_points_data_input = np.array(grid_points_data_input)

        grid_points_predictions = model_rk.predict(p=grid_points_data_input, x = grid_points_data_x)
    ...
    def name(self):
        return 'predict_raster05 with regression kriging parks and lden'

    def displayName(self):
        return 'Predict Raster05 with regression kriging parks and lden'
